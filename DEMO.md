...
## 第五幕：Keyword Lab（~40 秒，时间够就演——这是"网站不是道具"的铁证）
- 指 🔎 Keyword Lab 表格："Darwin 已经把这个产品的关键词机会全测算好了——难度、月搜索量、意图。"
- **点一个 KD 最低的 → ✍️ Generate blog**（~10 秒出稿）→
  "一篇带内链和外链证据的 SEO 文章。我可以直接改——" （随手改一个词）
- **🚀 Publish** → ~15 秒后绿条出现 →
  **点开链接**："这篇文章现在已经在公网上了，sitemap 已更新。
  这个网站不是截图，是一台正在长大的 SEO 机器。"
- ⚠️ 时间紧就砍这幕，留给 Q&A 当杀手锏现场演

## 谢幕（15 秒）
- 切到 **/archive**（Project Library）→
